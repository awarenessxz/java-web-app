...
import { faArrowsAltH, faTextWidth } from '@fortawesome/fontawesome-svg-core';
// AgGrid
...
class AgGridWrapper extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            isSimplifiedView: props.showSimplifiedView,
            error: '',
    ...
    getRowSelectionProps = () => {
        if (this.props.enableRowSelection) {
            return {
                rowSelection: this.props.enableRowSelection.multiRowSelection ? 'multiple' : 'single',
    ...
    // handle resize
    handleResizeColumnsToFitText = () => {
        const gridColumnApi = this.columnApi;
        const colIds = gridColumnApi.getAllColumns().map((col) => col.colId);
        gridColumnApi.autoSizeColumns(colIds);
    };

    // handle change in column definition
    handleColumnDefinitionChange = () => {
        this.setState({
            isSimplifiedView: !this.state.isSimplifiedView,
        });
        if (this.state.isSimplifiedView) {
            this.gridApi.setColumnDefs(this.props.columnDefs);
        } else {
            this.gridApi.setColumnDefs(this.props.simplifiedColumnDefs);
        }
    };
        ...
        return (
            <Toolbar data-testid="toolbar" style={{ background: this.props.toolbarColor }}>
                ...
                <div>
                    ...
                    {this.props.enableColumnResize && (
                        <Fragment>
                            <ToolButton
                                onClick={this.handleResizeColumnsToFitText}
                                icon={faTextWidth}
                                tooltipMsg="Resize to Fit Content"
                            />
                            <ToolButton
                                onClick={() => this.gridApi.sizeColumnsToFit()}
                                icon={faArrowsAltH}
                    ...
                    {this.props.enableRowSelection && (
                        <ToolButton
                            ...
                            buttonType="badge"
                            disabled={this.props.rowData.length <= 0}
                        />
                    )}
                    ...
                    {this.props.simplifiedColumnDefs !== undefined && (
                        <ToolButton
                            onClick={this.handleColumnDefinitionChange}
                            icon={() => {}}
                            tooltipMsg={this.state.isSimplifiedView ? 'Show original view' : 'Show simplified view'}
                            buttonType="switch"
                            switchGroup="columnDefinitionSwitch"
                            switchIsChecked={this.state.isSimplifiedView}
                        />
                    )}
                </div>
            </Toolbar>
                    ...
                    this.state.isWrapperReady && (
                        <Fragment>
                            ...
                            <div
                                ...
                                style={{ height: this.props.height, width: this.props.width, ...this.props.styles }}
                            >
                                <AgGridReact
                                    onGridReady={(params) => {
                                    ...
                                    columnDefs={
                                        this.props.showSimplifiedView
                                            ? this.props.simplifiedColumnDefs
...
AgGridWrapper.defaultProps = {
    enableSimplifiedView: undefined,
    ...
    simplifiedColumnDefs: undefined,
...
AgGridWrapper.propTypes = {
    ...
    columnDefs: PropTypes.arrayOf(
        PropTypes.shape({
            ...
            filter: PropTypes.onOfType([PropTypes.bool, PropTypes.string]),
    ...
    /** Simplified ColumnDefs (same as ColumnDefs) -- [Optional] */
    simplifiedColumnDefs: PropTypes.arrayOf(PropTypes.object),
    /** Show SimplifiedView (works concurrently with simplifiedColumnDefs */
    showSimplifiedView: PropTypes.bool,
